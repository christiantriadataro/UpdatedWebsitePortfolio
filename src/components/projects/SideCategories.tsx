...
const SideCategories = ({categories, isChecked, setIsChecked, currentCheck, setCurrentCheck}) => {
    ...
    return (
        <>
            ...
            <div className="w-full h-full xl:w-4/12 lg:w-3/12 border-r border-[#1E2D3D]">
                ...
                <div className={`${isOpen ? "" : "hidden"} transition duration-100 text-[13px] text-[#273E53]`}>
                    {categories.map((category, index) => (
                        <CheckBox title={Object.keys(category)[0]} icon={Object.values(category)[0]}
                                  onCheck={() => handleChecked(index)} isChecked={isChecked[index]} key={index}/>
                    ))}
                </div>
            </div>
        </>
